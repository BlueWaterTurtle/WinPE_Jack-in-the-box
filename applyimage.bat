...
::I've commented out the below lines because our project doesn't require a recovery partition. 
...
::The below line forces the "recovery veriable to "Y". This is to avoid the prompt only. 
...
::@Set READY=Y ::this line may need to be commented out. I may have added this to comment out the prompt and speed up the deployment. 
...
@if %Firmware%.==0x2. if %RECOVERY%.==Y. diskpart /s E:\CreatePartitions-UEFI.txt
...
::@SET /P EA=(Y or N):
::@if %EA%.==y. set EA=Y ******* I'm going to manually set the variable below here...
@SET COMPACTOS=N
@SET EA=N
